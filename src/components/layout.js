    ...
    return (
      <TopBarContainer frontpage={this.props.frontpage}>
        ...
        <Menu open={this.state.open}>
          ...
          <MenuLink onClick={()=>this.setButtonState(false)} to="/"><div>ryser</div><div>laboratory</div> </MenuLink>
          {structure.map(d=>(
          <MenuLink onClick={()=>this.setButtonState(false)} to={`/${d}/`}key={`link_${d}`}>{d}</MenuLink>
        ))}
        </Menu>
      </TopBarContainer>
    )
